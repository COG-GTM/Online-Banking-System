...
public class AccountServiceImpl implements AccountService {
    ...
    public void deposit(String accountType, double amount, Principal principal) {
        BigDecimal depositAmount = AmountValidator.validate(BigDecimal.valueOf(amount));
    ...
    public void withdraw(String accountType, double amount, Principal principal) {
        BigDecimal withdrawAmount = AmountValidator.validate(BigDecimal.valueOf(amount));
        User user = userService.findByUsername(principal.getName());

        if (accountType.equalsIgnoreCase("Primary")) {
            PrimaryAccount primaryAccount = user.getPrimaryAccount();
            if (primaryAccount.getAccountBalance().compareTo(withdrawAmount) < 0) {
                throw new IllegalArgumentException("Insufficient funds in the Primary account.");
            }
            ...
            if (savingsAccount.getAccountBalance().compareTo(withdrawAmount) < 0) {
                throw new IllegalArgumentException("Insufficient funds in the Savings account.");
            }
