...
public class AccountController {
    ...
    public String depositPOST(@ModelAttribute("amount") String amount, @ModelAttribute("accountType") String accountType, Principal principal, Model model) {
        Optional<BigDecimal> parsedAmount = MonetaryAmount.parse(amount);
        if (!parsedAmount.isPresent()) {
            return amountError(model, accountType, amount, "deposit");
        }
    ...
    public String withdrawPOST(@ModelAttribute("amount") String amount, @ModelAttribute("accountType") String accountType, Principal principal, Model model) {
        Optional<BigDecimal> parsedAmount = MonetaryAmount.parse(amount);
        if (!parsedAmount.isPresent()) {
            return amountError(model, accountType, amount, "withdraw");
        }
    ...
    private String amountError(Model model, String accountType, String amount, String view) {
        model.addAttribute("accountType", accountType);
        model.addAttribute("amount", amount);
        model.addAttribute("amountError", MonetaryAmount.ERROR_MESSAGE);
